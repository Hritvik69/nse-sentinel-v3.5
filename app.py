...
# ── Stock Aura engine (optional, graceful if missing) ─────────────────
try:
    from app_stock_aura_section import render_stock_aura_panel
    _STOCK_AURA_OK = True
except ImportError:
    _STOCK_AURA_OK = False

    def render_stock_aura_panel():  # type: ignore[misc]
        pass
...
if "results" in st.session_state:
    ...
    if results:
        ...
        try:
            ...
            if _cached_mb and (_now_ts - float(_cached_ts)) < 1800:
                _mb = _cached_mb
            else:
